    ...
    for i in range(5):
        for j in range(4):
            modpfx = os.path.join(
                ...
                cfg.DNN_MODEL_DIR,
                organism,
                "%s.%d.txt" % (cfg.DNN_MODELS[organism][i], j),
            )
...
def relevance(x, model, relout=2):
    ...
    hiddenLayerOut, mergedPoolOut, convLayerOut = (
        layer_outs[1][0],
        layer_outs[2][0],
        layer_outs[3][0],
    )
    ...
    hiddenLayerOut = Model(inputs=model.input, outputs=model.layers[-3].output).predict(
        x
    )[0]
    ...
    for i in range(2 * filters):
        for j in range(hiddens):
            Rp[i] += (
                Rh[j] * mergedPoolOut[i] * W[i, j] / numpy.dot(mergedPoolOut, W[:, j])
            )
    Rpm = Rp[:filters]
    Rpa = Rp[filters:]
    ...
    for i in range(filters):
        ...
        for j in range(window):
            for k in range(channels):
                F[i][j, k] = numpy.select(
                    [weights[0][j, k, i] > 0], [weights[0][j, k, i]]
                )
...
def predictsp(X, cls, organism, we, cpu=1):
    ...
    if len(P) > 0:
        ...
        for i in range(5):  # 5
            for j in range(4):  # 4
                crfmodel = os.path.join(
                    cfg.DEEPSIG_ROOT,
                    cfg.CRF_MODEL_DIR,
                    organism,
                    "model.seq.w%d.s%s.l%d.%d.%d"
                    % (
                        cfg.CRF_WINDOWS[organism],
                ...
                pred = runCRF(
                    ...
                    crfmodel,
                    cfg.CRF_WINDOWS[organism],
                    cfg.CRF_PARAMS[organism][i]["decoding"],
                    we,
                    cpu=cpu,
                )
                C.append(pred)
...
def get_json_output(acc, sequence, p_class, prob, cleavage):
    ...
    acc_json["sequence"] = {"length": len(sequence), "sequence": sequence}
    start = 1
...
def write_processed_sequences(
    records: List[SeqIO.SeqRecord],
    cls: List[int],
    cleavage: List[Union[int, str]],
    outfile: str,
) -> None:
...
def write_noss_sequences(
    records: List[SeqIO.SeqRecord], cls: List[int], outfile: str
) -> None:
    ...
    with open(outfile, "w") as f:
        for record, cl in zip(records, cls):
            if cl != 2:  # Not a SignalPeptide
                print(
                    f">{record.description}" if record.description else f">{record.id}",
                    file=f,
                )
